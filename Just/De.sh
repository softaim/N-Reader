...
echo -e "             \033[1;32m\033[5;37m\033[42m DETECT HEARTBLEED SSL Vulnerability \e[0m""                 "
echo _____________-------------------------------------_______________
echo
read -p "Please enter the IP Address or URL of  Target: " IP
echo
echo -e "\033[1;33m1.\033[1;32m Heartbleed Testing \e[0m"
echo
...
if [ $ch == 1 ]
then
nmap -Pn -sV --script=ssl-heartbleed $IP > $path
fi
echo -e "\e[1;33mAll the output has been saved to: $path \e[0m"
elif [ "$save" == 1 ]; then
if [ $ch == 1 ]
then
nmap -Pn -sV --script=ssl-heartbleed $IP
fi
else
chmod +x N.sh
./N.sh
