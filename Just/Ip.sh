...
echo -e "             \033[1;32m\033[5;37m\033[42m IP Address Information \e[0m""                 "
echo _____________-------------------------_______________
echo
read -p "Please enter the IP Address or URL of  Target: " IP
echo
echo -e "\033[1;33m1.\033[1;32m Find Information about IP address \e[0m"
echo
...
if [ $ch == 1 ]
then
nmap -Pn --script=asn-query,ip-geolocation-maxmind $IP > $path
fi
echo -e "\e[1;33mAll the output has been saved to: $path \e[0m"
elif [ "$save" == 1 ]; then
if [ $ch == 1 ]
then
nmap -Pn --script=asn-query,ip-geolocation-maxmind $IP
fi
else
chmod +x N.sh
./N.sh
